...
class AdaptadorElementos(val ListaElementos:ArrayList<Elementos>): RecyclerView.Adapter<AdaptadorElementos.ViewHolder>() {
    ...
    class ViewHolder (itemView : View): RecyclerView.ViewHolder(itemView) {
        val fImagen = itemView.findViewById<ImageView>(R.id.elemento_img);
        val fNombre = itemView.findViewById<TextView>(R.id.elemento_usuario)
        val fCategoria = itemView.findViewById<TextView>(R.id.elemento_estado)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder?.fCategoria?.text=ListaElementos[position].categoria
        ...
        holder.itemView.setOnClickListener(){
            val llamaraactividad = Intent(holder.itemView.context,verPelicula::class.java)
            llamaraactividad.putExtra("id",id.toString())
            llamaraactividad.putExtra("nombre",nombre.toString())
            llamaraactividad.putExtra("categoria",categoria.toString())
            llamaraactividad.putExtra("duracion",duracion.toString())
            llamaraactividad.putExtra("imagen",imagen.toString())
            holder.itemView.context.startActivity(llamaraactividad)
        }
    }
